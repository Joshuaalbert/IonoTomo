...
def invertSingleTime(dataPackObs,numThreads,datafolder,straightLineApprox=True,antIdx=np.arange(10),timeIdx=[0],dirIdx=np.arange(10)):
    ...
    muTCI.m = mu
    ##### define bfgs
    #function is misfit (or R?)
    def misfit(x,*args):
        dobs,dataPack,refAnt,muTCI,numThreads,rays,K_e,sigma_ne_factor,Cd,antIdx,timeIdx,dirIdx = args
        muTCI.m = x
        muTCI.clearCache()
        antennas,antennaLabels = dataPack.get_antennas(antIdx = antIdx)
        patches, patchNames = dataPack.get_directions(dirIdx=dirIdx)
        ...
        dd = np.zeros(Na*Nt*Nd,dtype=np.double)
        CdCt = np.zeros(Na*Nt*Nd,dtype=np.double)
        k = 0
        while k < Nd:
            c = 0
            j = 0
            while j < Nt:
                ...
                while i < Na:
                    ...
                    dd[h] = dobs[i,j,k] - dtec_threads[k][c]
                    CdCt[h] = Ct_threads[k][c] + Cd[i,j,k]
                    c += 1
                    i += 1
                j += 1
            k += 1
        dataPack.set_dtec(dtec,antIdx=antIdx,timeIdx=timeIdx, dirIdx=dirIdx,refAnt=refAnt)
        d = dataPack.get_dtec(antIdx=antIdx,timeIdx=timeIdx, dirIdx=dirIdx)
