    ...
    def _build_sgp_model(self, sess, weights, X, Y, ls_scale, y_scale, 
        ...
        with gp.defer_build():
            k_time = gp.kernels.RBF(1,active_dims = [0],
                                    lengthscales=[0.5 if ls_init[0] is None else ls_init[0]/ls_scale[0]],variance=0.88)
            k_space = gp.kernels.RBF(2,active_dims = [1,2],
                                    lengthscales=[1.0 if ls_init[1] is None else ls_init[1]/ls_scale[1]],variance=0.88)
...
#             plt.show()
            ...
            with tf.Session(graph=tf.Graph()) as sess:
                ...
                opt = gp.train.AdamOptimizer(5e-2)
    ...
    def solve_and_apply_ensemble(self, save_datapack, ant_idx, time_idx, dir_idx, freq_idx, iterations,
        ...
        with futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
            ...
            for j,aj in enumerate(time_idx[::shift]):
                ...
                #logging.warning("Initial temporal corr {:.4f} directional corr {:.4f}".format(*model_kwargs['ls_init']))
...
if __name__=='__main__':
    ...
    model_kwargs = {'minibatch_size':250, 'M':40*15,'feature_trainable':False,'ls_init':(73,0.30),
    ...
    smoothing.solve_and_apply_ensemble(starting_datapack.replace('.hdf5','_smoothed_ensemble.hdf5'),
                                       [50,51], -1, -1, -1, iterations=200,
                                 interval = 30, shift = 6, init_solutions=None,#'../../bayes/gp_params_fixed_scales.npz',
                                       num_threads=2,verbose=True,model_kwargs = model_kwargs)
