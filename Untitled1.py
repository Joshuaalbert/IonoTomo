...
def invertSingleTime(dataPackObs,numThreads,datafolder,straightLineApprox=True,
    ...
    refAntIdx = 0
    zmax = 1000.
    L_ne,sigma_ne_factor = 20.,0.2
    # get setup from dataPack
    dataPack = dataPackObs.clone()
    
#     getant = delayed(partial(dataPack.get_antennas,antIdx = antIdx))
#     antennas,antennaLabels = getant[0],getant[1]
#     getdir = delayed(partial(dataPack.get_directions,dirIdx = dirIdx))
#     patches, patchNames = getdir[0],getdir[1]
#     gettime = delayed(dataPack.get_times,timeIdx=timeIdx)
#     times,timestamps = gettime[0],gettime[1]
